...
#    dirf = '%s/model%s-ts-001'%(project_dir,count)
    ...
    newpdb = '%s/m1pdbfiles/%d.pdb'%(dirf,pdb_name)
...
#    os.chdir('%s'%work_dir)
    system_run('cp %s/1.inp %s/modred1.inp'%(dirf,dirf))
    system_run('cp %s/1.out %s/modred1.out'%(dirf,dirf))
    system_run('cp %s/1.chk %s/modred1.chk'%(dirf,dirf))
    os.chdir('%s'%dirf)
    input = open('1.inp','w')
...
#    input.write("#P b3lyp/genecp opt(ts,noeigen,calcfc) freq scf=(xqc,maxcon=128,maxcyc=128)\n")      #many things can be changed to user input
    ...
    for i in range(len(new_pdb)):
        line = tmp_pdb[i]
        xyz = new_pdb[i]
        if 'SD' in line[2] and 'SAM' in line[4]:
            mod_idx.append(i+1)
        if 'O2' in line[2] and 'DNC' in line[4]:
            mod_idx.append(i+1)
        input.write("%4s %6s %8.3f %8.3f %8.3f\n"%(line[14],line[16],xyz[8],xyz[9],xyz[10])) 
    
    input.write("\n")
    input.write("%d %d F\n"%(mod_idx[0],mod_idx[0]+1))
    input.write("%d %d F\n"%(mod_idx[0]+1,mod_idx[1]))
    input.write("\n")
    input.write("Mg     0                              \n")
