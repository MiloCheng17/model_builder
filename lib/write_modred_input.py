...
#    dirf = '%s/model%s-ts-001'%(project_dir,count)
    ...
    newpdb = '%s/hpdbfiles/%d.pdb'%(dirf,pdb_name)
...
#    os.chdir('%s'%work_dir)
    system_run('cp %s/1.inp %s/relax-h.inp'%(dirf,dirf))
    system_run('cp %s/1.out %s/relax-h.out'%(dirf,dirf))
    system_run('cp %s/1.chk %s/relax-h.chk'%(dirf,dirf))
    os.chdir('%s'%dirf)
    input = open('1.inp','w')
